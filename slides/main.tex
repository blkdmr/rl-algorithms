...
MCTS incrementally builds a partial search tree from the current state,
using simulated trajectories to estimate action values and guide long-term decision making.

\vspace{0.25cm}
\textbf{Four Phases (one MCTS iteration)}
\begin{enumerate}
    \item \textbf{Selection:} starting from the root, recursively select child nodes by balancing
    exploitation of high-value actions and exploration of less-visited ones.
    \item \textbf{Expansion:} when a non-terminal node with unexplored actions is reached,
    expand the tree by generating a new child node.
    \item \textbf{Simulation (Rollout):} from the newly expanded node, simulate the environment forward
    using a simple policy to estimate future reward.
    \item \textbf{Backpropagation:} propagate the simulated return back through the visited nodes,
...
\begin{frame}{Results for CartPole}

\centering
\includegraphics[width=0.7\textwidth]{images/results/cart_pole.png}
...
\centering
\includegraphics[width=0.5\textwidth]{images/results/lunar_lander.png}
...
\centering
\includegraphics[width=0.5\textwidth]{images/results/galaxian.png}
